...
#     pylab.show()
    ...
    return t_init , t_exec * dt / Simulation_Time  
...
if __name__ == '__main__':
    ...
    if in_benchmark:
        with open('result.log', 'a') as f:
            if Use_MasterHSolve:
                f.write( "\t" +str(ti)+"\t"+str(te)+ "\n")
            else:
                f.write( sys.argv[2] +"\t" + os.environ['PN2S_MP_SIZE'] + "\t")
                f.write(os.environ['MAX_STREAM_NUMBER'] + "\t"+str(ti)+"\t"+str(te))
    else:
        print(str(ti)+"\t"+str(te))
            
    
# 
#     for number_of_ext_cells in range(1,10):
#         
#         print("--- Ex: " + str(number_of_ext_cells) + "\t t=" + str(t_exec) )
