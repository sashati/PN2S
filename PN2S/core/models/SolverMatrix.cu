...
template<> class config<double,ARCH_SM30> {
public:
    enum { fsMinDim   =    2 };
    enum { fsMaxDim   =   76 };
...
template<> class config<float,ARCH_SM30> {
public:
    enum { fsMinDim   =    2 };
    enum { fsMaxDim   =   76 };
...
//    if (ty == n) x[tx] = As(tx,n);
    if (ty == n)   x[tx] = ((T)2.0) * As(tx,n) - x[tx] ;
}
...
template <typename T, int arch>
int SolverMatrix<T,arch>::fast_solve (const T *A_d, T *b_d, T *x_d, int n, int batch, cudaStream_t stream)
{
	int matinv = 0;
...
template class SolverMatrix<double, ARCH_SM13>;
template class SolverMatrix<double, ARCH_SM20>;
template class SolverMatrix<double, ARCH_SM30>;
template class SolverMatrix<double, ARCH_SM35>;
template class SolverMatrix<float, ARCH_SM13>;
template class SolverMatrix<float, ARCH_SM20>;
template class SolverMatrix<float, ARCH_SM30>;
template class SolverMatrix<float, ARCH_SM35>;
