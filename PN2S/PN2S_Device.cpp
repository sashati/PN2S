...
int _iq_size;
int _iq_limit;
std::deque<PN2S_TaskInfo> _iq;

//No limit size for output queue
int _oq_size;
std::deque<PN2S_TaskInfo> _oq;
...
void PN2S_Device::task1_prepareInput(omp_lock_t& _empty_lock,omp_lock_t& _full_lock) {
	for (vector<PN2S_ModelPack>::iterator it = _solver.modelPacks.begin(); it != _solver.modelPacks.end(); ++it)
	{
		...
		#pragma omp critical
		{
			...
			cout<<"Thread = "<< omp_get_thread_num()<<"Limit "<<_iq_limit<<" Add   B"<<t.type<<"  QSize="<<_iq.size()<<"size="<<_iq_size<<endl<<flush;
		}
		...
		omp_lock_t& _empty_lock_output)
{
	for (int i = 0; i < 10; i++) {
		...
		#pragma omp critical
		{
			_iq_size--;
			_oq_size++;
			cout<<"Thread = "<< omp_get_thread_num()<<"Limit "<<_iq_limit<<" Process B"<<t.type<<"  QSize="<<_iq.size() <<"size="<<_iq_size<<endl<<flush;
		}
...
void PN2S_Device::task3_prepareOutput(omp_lock_t& _empty_lock) {
	...
	//	#pragma omp flush(_list)
		...
		{
			_oq_size--;
			cout<<"Thread = "<< omp_get_thread_num()<<" Output B"<<res->type<<"  QSize="<<_oq.size() <<"size="<<_oq_size<<endl<<flush;
		}
	}
}
